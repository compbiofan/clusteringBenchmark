    ...
    if cluster_algo == 'bnpc':
        ...
        for line in lines:
            ...
            with open(op_fname+"."+str(count)+".slurm",'w') as f:
                f.write("#!/bin/bash\n")
                f.write("#SBATCH --job-name=bnpc_job"+str(count)+"\n")
                f.write("#SBATCH -N 1\n")
                f.write("#SBATCH -n "+str(n)+"\n")
                f.write("#SBATCH --mem-per-cpu="+mpc+"\n")
                f.write("#SBATCH -p "+p+"\n")
                f.write("#SBATCH --mail-type=ALL\n")
                f.write("#SBATCH --output=/gpfs/research/fangroup/rk18g/longitudinal/clustering/bnpc"+opdir_name+"/bnpc_output"+str(count)+".out\n")
                f.write("#SBATCH --error=/gpfs/research/fangroup/rk18g/longitudinal/clustering/bnpc"+opdir_name+"/bnpc_error"+str(count)+".out\n")
                f.write(line)
            count=count+1
    elif cluster_algo == 'scclone':
        with open(fname,'r') as f:
            lines = f.readlines()
        ...
        for line in lines:
            if "#" in line:
                continue
            line_items = line.split(" ")
            #opdir_name = (((line_items[3].strip()).replace(".","")).split("/"))[2]
            opdir_name = line_items[3].strip()
            print(opdir_name)
            with open(op_fname+"."+str(count)+".slurm",'w') as f:
                f.write("#!/bin/bash\n")
                f.write("#SBATCH --job-name=scg_job"+str(count)+"\n")
                f.write("#SBATCH -N 1\n")
                f.write("#SBATCH -n "+str(n)+"\n")
                f.write("#SBATCH --mem-per-cpu="+mpc+"\n")
                f.write("#SBATCH -p "+p+"\n")
                f.write("#SBATCH --mail-type=ALL\n")
                f.write("#SBATCH --output=/gpfs/research/fangroup/rk18g/longitudinal/clustering/scg/"+opdir_name+"/scg_output"+str(count)+".out\n")
                f.write("#SBATCH --error=/gpfs/research/fangroup/rk18g/longitudinal/clustering/scg/"+opdir_name+"/scg_error"+str(count)+".out\n")
                f.write(line)
            count=count+1
