...
def distribute_SNVcells(cell_n, tree_f, out_f, eta):
    ...
    dbl_dict = {}
    eta = 0 #because we are not using doublet here
    ...
    for i in range(len(SNVcell_nums)):
        ...
        if SNVcell_num != 0:
            ...
            #print(" SNVcell_IDs ",SNVcell_IDs)
            #print(" prev ",prev)
            #print(str(prev + 2))
            #print(str(prev + int(SNVcell_num) + 1))
            for j in range(prev + 2, prev + int(SNVcell_num) + 1):
...
# Get allele dropout rate. 
def get_allele_dropOut(ado_rate):
    toss_random_no = random.random() # Toss a random number to determine allele dropout.
    if toss_random_no < ado_rate:
        return True
    else:
        return False

# Get the total read count matrix.
def get_site_totalRC(G, n, m, coverage):
    total_rc = init(n,m) # Total read count matrix
    for i in range(len(G)):
        for j in range(len(G[i])):
            random_read_count = random.random()# toss a random read count between [0,1]
            if random_read_count <= coverage and random_read_count > (coverage * coverage):
                total_rc[i][j] = 1
            elif random_read_count <= (coverage * coverage):
                total_rc[i][j] = 2
    #print_matrix(total_rc, n, "total_rc_matrix.csv")
    return total_rc

# get the read count matrix from G based on coverage. 
def add_read_count(G, n, m, alpha, coverage, ado_rate):
    # For G[i,j] = 0 it is pretty straightforward.
    # For G[i,j] = 1 we need to check ADO.
    # Total read count t is an array. We check if each i in t is a variant and get variant read count v.
    total_rc_matrix = get_site_totalRC(G, n, m, coverage)
    variant_rc_matrix = init(n,m)
    for i in range(len(G)):
        #print(" No of cells len ",len(G)) 
        for j in range(len(G[i])):
            if total_rc_matrix[i][j] >= 1:
                if G[i][j] == 0:
                    total_variant = 0 # check if a read is variant read.
                    for k in range(total_rc_matrix[i][j]): # we want to check if each read count in total read count is a variant read
                        v_random = random.random() # toss a random number to check variant read based on FP.
                        if v_random < alpha:
                            total_variant = total_variant+1
                    #print(" Total rc ",str(total_rc_matrix[i][j])," variant reads ",str(total_variant))
                    variant_rc_matrix[i][j] = total_variant
                if G[i][j] == 1:
                    #print(" When G is 1 ")
                    ado = get_allele_dropOut(ado_rate) # Get the allele dropout rate
                    #print(" Allele dropout happened ",ado)
                    if ado:
                        check_variant_read = random.random() # We toss a random number and check for variant allele.
                        if check_variant_read >= 0.5: # then both alleles are variant alleles.
                            variant_rc = total_rc_matrix[i][j]
                            variant_rc_matrix[i][j] = variant_rc
                        else:
                            variant_rc_matrix[i][j] = 0 # variant read count is 0 because both alleles are reference allele.
                    else: # Allele dropout did not happen
                        v_count = 0
                        for k in range(total_rc_matrix[i][j]):
                            random_v_count = random.random()
                            if random_v_count < 0.5:
                                v_count = v_count+1
                        variant_rc_matrix[i][j] = v_count
    print_matrix(variant_rc_matrix, n, "variant_rc_matrix.csv")
    #total_zeros = count_total_value(variant_rc_matrix, n, m, 0)
    #total_ones = count_total_value(variant_rc_matrix, n, m, 1)
    #total_twos = count_total_value(variant_rc_matrix, n, m, 2)
    #print("total zeros for variant RC: " + str(total_zeros) + "; total ones for variant RC: " + str(total_ones) + "; total twos for variant RC: "+ str(total_twos))
    #print("-------------------------------")
    #total_zeros = count_total_value(total_rc_matrix, n, m, 0)
    #total_ones = count_total_value(total_rc_matrix, n, m, 1)
    #total_twos = count_total_value(total_rc_matrix, n, m, 2)
    #print("total zeros for total RC: " + str(total_zeros) + "; total ones for total RC: " + str(total_ones) + "; total twos for variant RC: "+ str(total_twos))
    return total_rc_matrix, variant_rc_matrix

def get_Dmatrix(t_matrix, v_matrix, n, m, D_matrix_file):
    D_matrix = init(n,m)
    for i in range(len(t_matrix)):
        for j in range(len(t_matrix[i])):
            ...
            if t_matrix[i][j] == 0:
                D_matrix[i][j] = 3
            if t_matrix[i][j] >= 1 and v_matrix[i][j] == 0:
                D_matrix[i][j] = 0
    print_matrix(D_matrix, n, D_matrix_file)
    return D_matrix

def get_sbmclone_input(D_matrix, sbmclone_file):
    sbmclone_file = open(sbmclone_file, "w")
    for i in range(len(D_matrix)):
        for j in range(len(D_matrix[i])):
            if D_matrix[i][j] == 1:
                print(str(i)+","+str(j), file = sbmclone_file)
    sbmclone_file.close()

# from tree_f we know which cells (on leaf nodes) have which mutations (on edges)
def mutation_matrix(mut_array, SNVcell_array, tree_f, n, m, missingP, alpha, beta, coverage, ado_rate, G_matrix_file, D_matrix_file, sbmclone_file):
    ...
    for i in range(len(SNVcell_array)):
        ...
        mutID_array = []
        # print(mut_array_dict)
        #print(" Edges above leaf ",edges_above_leaf)
        for e in edges_above_leaf[0]:
            #print(e)
    ...
    # We need the G matrix till now. Based on the coverage and read count do the modifications below.
    # We need to traverse the G matrix and include conditions for when G=0 and G=1. 
    # We won't be needing the below code to get missing data, False positive, False negative rates.
    
    add_read_count(G, n, m, alpha, coverage, ado_rate)
    total_rc_matrix, variant_rc_matrix = add_read_count(G, cell_n, mut_n, alpha, coverage, ado_rate)
    D_matrix = get_Dmatrix(total_rc_matrix, variant_rc_matrix, cell_n, mut_n, D_matrix_file)
    get_sbmclone_input(D_matrix, sbmclone_file)
...
if len(sys.argv) <= 1:
    ...
    Usage: python sim_par.py -a [alpha] -b [beta] -m [missing-rate] -c [num-cells] -n [num-mutations] -f [input-tree-file] -P [prefix-output-files]
        ...
        -cov (--coverage)   Coverage. [0.03]
        -ado (--ado_rate)   Allele dropout rate. [0.2]
...
parser.add_argument('-e', '--doublet-rate', default=0.0)
parser.add_argument('-cov', '--coverage', default=0.03)
parser.add_argument('-ado', '--ado_rate', default=0.2)
...
mutation_matrix(mut_array, SNVcell_array, tree_f, cell_n, mut_n, missing, alpha, beta, coverage, ado_rate, prefix + ".G.csv", prefix + ".D.csv", prefix + ".miss.csv")
